...
def _meta(asset_service: AssetService, sources: list[str] | None = None) -> dict:
    provider_names = sources or asset_service.provider_names
    return {
        "sources": provider_names,
        "fetched_at": datetime.now(timezone.utc),
        "cache_hit": False,
        "is_demo": bool(provider_names) and provider_names[0].lower() == "demo",
        "warnings": [],
    }
...
def search_instruments(q: str, asset_service: AssetService = Depends(get_asset_service)):
    ...
    return {"data": [r.model_dump() for r in results], "meta": _meta(asset_service)}
...
def get_profile(code: str, asset_service: AssetService = Depends(get_asset_service)):
    ...
    return {"data": profile.model_dump(), "meta": _meta(asset_service, asset_service.profile_sources(code))}
...
def get_instrument(code: str, asset_service: AssetService = Depends(get_asset_service)):
    try:
        ...
        return {"data": instrument.model_dump(), "meta": _meta(asset_service)}
